        ...
        if (getItemViewType(position) == PictureConfig.TYPE_CAMERA) {
            ...
            if (config.enablePreview || config.enPreviewVideo || config.enablePreviewAudio) {
                contentHolder.btnCheck.setOnClickListener(v -> {
                    ...
                    boolean isVideo = PictureMimeType.isHasVideo(image.getMimeType());
                    if (isVideo) {
                        long duration = image.getDuration();
                        long size = image.getSize();
                        if (duration > 5 * 60 * 1000) {
                            showPromptDialog("发布视频时长不得超过5分钟");
                            return;
                        } else {
                            if (size > 100 * 1024 * 1024) {
                                showPromptDialog("发布视频大小不得超过100M");
                                return;
                            }
                        }
                    }
...
//                        showPromptDialog("发布视频时长不得超过3分钟");
//                        return;
//                    } else {
//                        if (size > 60 * 1024 * 1024) {
//                            showPromptDialog("发布视频大小不得超过60M");
//                            return;
//                        }
//                    }
//                }
